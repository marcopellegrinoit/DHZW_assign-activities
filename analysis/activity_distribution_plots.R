...
## This script plot activityp type distribution for the whole week and per day
...
df_comparison.activities_all$dataset <- recode(
  ...
  'DHZW' = paste0('DHZW ODiN (', length(unique(df_DHZW$ODiN_ID)),' pp)'),
  'pool' = paste0('Extremely Urbanised ODiN \n (', length(unique(df_pool$ODiN_ID)),' pp)'),
  'synthetic' = paste0('Synthetic Population \n (', length(unique(df_synthetic$agent_ID)),' pp)')
)

df_comparison.activities_all$proportion = df_comparison.activities_all$proportion * 100
...
plot_all <- ggplot(df_comparison.activities_all, aes(activity, proportion)) +
  ...
  ggtitle("Activity distribution evaluation") +
  ylab("Proportion (%)") +
  xlab("Activity") +
...
for (i in c(1:7)) {
  ...
  df_DHZW.activities_day$DHZW = df_DHZW.activities_day$DHZW * 100
  ...
  df_pool.activities_day$pool = df_pool.activities_day$pool * 100
  ...
  df_synthetic.activities_day$synthetic = df_synthetic.activities_day$synthetic * 100
  ...
  df_comparison.activities_day$dataset <- recode(
    ...
    'DHZW' = paste0('DHZW ODiN (', length(unique(df_DHZW[df_DHZW$day_of_week==i,]$ODiN_ID)),' pp)'),
